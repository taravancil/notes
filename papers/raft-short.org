...
* Questions
- (answered) When there's a split vote, how is that broadcast to candidates?
It's not! In the case that there's a split vote, neither candidate
becomes the leader and a new election begins.
- (answered) At what exact point is an entry committed in a leader's server?
...
- (answered) Figure 8. What do authors mean by entries that are committed
  indirectly?
...
- (answered) If a follower crashes after it's committed an entry but before it
